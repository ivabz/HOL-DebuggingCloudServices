...
function RenumberDemo() {
    ...
    if (tables.length > 0) {
        for (t = 0; t < tables.length; t++) {
            ...
            if ((table.innerText 
                 && table.innerText.substr(0, chkstr.length) == chkstr
				)
				||
				(headers 
				&& headers.length == 3
				&& headers[0].textContent.trim() == 'Action'
				&& headers[1].textContent.trim() == 'Script'
				&& headers[2].textContent.trim() == 'Screenshot'
				)) {
				...
				for (r = 0; r < trs.length; r++) {
					...
					for (c = 0; c < tds.length; c = c + 3) {
						var ols = tds[c].childNodes;
						for (i = 0; i < ols.length; i++) {
							if (ols[i].nodeName == 'OL') {
